...
First, skim this [Wikipedia article](https://en.wikipedia.org/wiki/Definite_matrix).

This article, in all its length, somehow manages to glance over
...
of a matrix transform as the amount it scales area by, we can see
...
will be positive. Otherwise, it must either be less than 0, which corresponds
